#!/bin/bash
# scripts/build-win.sh — build Ellamaka Desktop and/or CLI for Windows via CI.
#
# Dispatches the selected workflow(s) on the current branch with publish=false,
# waits for completion, downloads the Windows artifact(s), and prints run links.
#
# Usage:
#   build-win.sh [desktop|cli] [options]
#
# Subcommands:
#   desktop   Build Windows Desktop (default when no subcommand)
#   cli       Build Windows CLI
...
show_help() {
  cat <<'EOF'
Usage: build-win.sh [desktop|cli] [options]

Build the Ellamaka Desktop and/or CLI for Windows via CI, without publishing,
then download the Windows artifact(s). With no subcommand, both are built.
...
build_one() {
  ...
  if [[ "$target" == "desktop" ]]; then
    ...
    echo "Building Windows Desktop (channel: $CHANNEL, version: $version, branch: $BRANCH)"
  else
    ...
    echo "Building Windows CLI (web UI: $WEB_UI, version: $version, branch: $BRANCH)"
  fi
  ...
  if [[ "$target" == "desktop" ]]; then
    local exe
    exe="$(flatten_artifact "ellamaka-desktop-win-x64.exe" || true)"
    if [[ -z "$exe" ]]; then
    ...
    if [[ -z "$cli_archive" ]]; then
      echo "ellamaka-windows-x64.zip not found in downloaded artifact" >&2
      find "$OUT_DIR" -type f | head -20 >&2
      return 1
    fi
