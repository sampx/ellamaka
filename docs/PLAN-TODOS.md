...
> **历史说明**：本文档前身为 `POC-cordis-todos.md`（2026-08-16）→ `PLAN-TODOS.md`（2026-08-19 终审重构）→ 本次（2026-08-20）按 dsh 双引擎融合实验重写。历史设计转向（cordis 主线 → Effect 原生主线）已抹去，回到 cordis 微内核愿景 + 双引擎现实。
...
| 日期 | Plan | 记录 |
|------|------|------|
| 2026-08-16 | — | 文档创建（原名 POC-cordis-todos.md）；Plan 1–7 规划定稿 |
| 2026-08-17 | Plan 1 | Plan 1 实施完成（cordis 容器宿主 + spill 挂载）；用户验证通过 |
| 2026-08-19 | — | 路线终审：cordis 降级为边缘线，主线切换 Effect 原生能力路线（`DESIGN-capabilities.md`） |
| 2026-08-20 | — | **设计转向抹去**：删除 `DESIGN-refactor-cordis.md` + `DESIGN-capabilities.md`，回到 cordis 微内核愿景 + 双引擎现实。新设计 `DESIGN-dsh-poc.md`（边实践边设计、桥/吸收双轨、微内核留白）。本文档按实验步骤重写（核心到外围）。PoC 定位为长期实验，不合并 main |
