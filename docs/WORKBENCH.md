...
- 运行中保持展开，并限制为约 120px 的可滚动预览，持续显示最新内容。
...
长对话在时间线左侧提供常驻的 `PromptNavigator`。该导航是 Ellamaka 在 Kilo Code 转录结构之上新增的能力，让用户从任意位置快速浏览并跳转到历史用户请求，同时使用 Ellamaka Workbench 的视觉 token 完成呈现。
...
- 每条已加载用户消息对应一个水平刻度。刻度按当前已加载对话范围内的相对位置垂直分布，而不是按虚拟列表当前挂载节点计算；历史分页完成后，该范围即完整会话。
- 当前视口所在对话轮使用较长、较强的刻度。其他消息使用较短、较弱的刻度。包含错误、未读结果或仍在运行的对话轮可以叠加语义状态标识。
- 点击刻度直接将对应用户消息滚动到时间线中央。拖动或连续点击不会破坏虚拟列表测量缓存。
- Hover、键盘聚焦或点击轨道空白区域打开消息目录浮层。轨道在消息数量较少时保持低对比度，在长对话中自然成为导航锚点。

##### PromptNavigationPopover：浮层消息目录

浮层从刻度轨右侧展开，覆盖消息内容而不引发布局重排。目标宽度为 360px，并受当前 Panel 可用宽度约束；最大高度约为 Chat 视口的 70%，内部独立滚动。

浮层结构如下：

1. **标题栏**：显示“提示词导航”，右侧提供上一条与下一条按钮。按钮围绕当前活动消息移动，并同步更新主时间线位置。
2. **消息目录**：每条用户消息形成一个目录项。
