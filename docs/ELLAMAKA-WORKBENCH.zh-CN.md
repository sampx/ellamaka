...
- **会话行左侧标识与置顶 (Pin)**：
  - 会话行左侧保留单一图标位，形状表示 Session marker 类型，颜色表示 Panel 绑定状态：
    ...
    - 未绑定为 `text-v2-icon-icon-muted`（灰），已绑定为 `text-v2-icon-icon-accent`（绿，与终端存活指示同色系）。
...
## 7. 异常处理与健壮自愈机制 (Robust Exception Handling & Recovery)

为了保证 Workbench 在各种运行状况（网络抖动、CLI 版本变化、环境配置损坏）下均能稳定可靠运行，系统必须遵循本章的异常防御与诊断设计。

### 7.1 异常分类与应对原则

| 严重性等级 | 典型场景 | 前端渲染表现 | 自愈/应对机制 |
| :--- | :--- | :--- | :--- |
| **致命错误 (Fatal)** | 后端 API Server 完全离线 / 应用根生命周期异常 | 全局 `ErrorPage` (最后的兜底) | 停止所有交互，引导用户“重新启动”或“查看/导出日志”。 |
| **空间/连通性错误 (Blocker)** | WopalSpace 根目录丢失 / 核心 Space API 连接中断 | 侧边栏/工作区遮罩错误页 | 拦截后续操作，防止产生无效状态或脏持久化数据。启动长轮询尝试自动连接重试。 |
| **版本契约冲突 (Incompatible)** | `wopal-cli` 的 `apiVersion` 契约版本不匹配 | 顶部全局 Diagnostic Banner | 允许用户继续当前操作（如继续本地 terminal）。横条上明确提示版本冲突（如：`cli >= 0.3.3`），提供“自升级”指引。 |
| **局部组件错误 (Warning)** | 空面板选择受控目录时 `locations` 接口拉取失败 | 局部 Inline 错误信息 + 局部重试 | **严禁抛出至父级 ErrorBoundary**！仅将错误局限在下拉选择框等局部控件区域，其余聊天/终端功能完全不受影响。 |

### 7.2 前端局部防御 (Granular Reactivity Safeguard)

为避免 SolidJS 的 `createResource` 异步请求错误向上击穿整个面板：
1. **安全只读 Memo**：在读取资源信号时，必须使用只读的 `createMemo` 包裹一层安全边界。当 `resource.error` 存在时直接返回安全默认值（如空数组 `[]`），从活性机制上拦截错误传播。
2. **就近降级渲染**：使用 `<Show when={locations.error}>` 就近展示红色的局部警示，同时显示捕获的详细底层错误信息（非技术黑话，例如“WopalSpace 未正确初始化”），并保留一个局部的重试动作。
3. **保留未受影响的操作**：出错时，面板内的其他非依赖性交互（如不需要读取该资源的独立会话创建、手动新建 PTY 终端等）应继续保持可用。

### 7.3 版本与连接探测握手 (Startup Handshake)

系统建立后端 CLI 的状态和版本握手链路：
1. **握手检查**：服务端扩展 `/global/health`（或专用握手端点），不仅验证服务端进程存活，还需执行 `wopal --version` 以检测 CLI 状态：
   - `ok`：一切正常。
   - `missing`：CLI 可执行文件不存在。
   - `incompatible`：CLI 版本与服务端所需的 `MIN_WOPAL_CLI_VERSION` 不兼容。
2. **前端 Banner 提示**：前端在侧边栏上方或顶栏渲染健康状况。一旦 CLI 状态非 `ok`，即刻渲染全局 Banner：
   - 包含明确解决方案（例如：“请在终端中运行 `wopal update` 升级 CLI”）。
   - 不阻塞当前的本地连接和已有的静态数据展现。

### 7.4 自动恢复与重试机制 (Resilient Auto-Recovery)

1. **瞬态错误自动恢复**：由于网络瞬间抖动导致的 API 调用失败（如 `fetchSessionTree` 或 `refreshSpaces`），前端底层采用指数退避重试（Exponential Backoff with jitter）自动拉取，在网络恢复后自动刷平状态，无需用户手动刷新网页。
2. **状态保护防御**：对于导致系统状态极度不一致的错误（如 Session 写入本地 SQLite 彻底失败），必须进行彻底的回滚（Rollback），并利用全局 `ErrorPage` 阻止用户进行可能带来“脏数据”的输入行为。
