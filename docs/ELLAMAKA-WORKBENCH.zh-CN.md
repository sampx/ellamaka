...
### 7.2 网络断连与模态保护 (Modal Overlay Protection)
...
### 7.4 局部非阻塞错误与状态栏诊断 (Status Bar Diagnostics)

为防止非阻塞的边缘 API 错误击穿整个大面板：
1. **防爆隔离**：对 SolidJS 的 `createResource` 资源拉取使用 `createMemo` 包裹安全读取，在 `error` 发生时返回空值，绝对禁止冒泡引起 ErrorBoundary 卸载面板。
2. **状态栏全局通知区**：在全局 Store 中维护一个极简的错误状态（`activeError: { message: string, onRetry?: () => void } | null`）。任何非阻塞接口出错时直接写入该状态。
3. **精简 UI 展现**：在常驻的状态栏路径提示旁，划出一段通知区。一旦有错，在此以 11px Muted 灰色文字渲染简短说明，并附带小型的 **“重试”** 和 **“清除”** 按钮。这保证了即使侧边栏折叠，用户也随时可见并一键重试。
