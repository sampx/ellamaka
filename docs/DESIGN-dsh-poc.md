...
**阅读地图**：§2 架构总览 → §3 运行时机制 → §4 能力采用 → §5 配置与隔离 → §6 已验证事实 → §7 设计约束 → §8 物化验收基线（已达成）→ §9 插件供应链（已达成）→ §10 workbench × dsh 前端插件互通（目标设计）。
...
> 状态：已于 2026-09-02 全部达成（Plan `feature-ellamaka-dsh-plugin-supply-chain-install-hot-mount-lifecycle`，200 测试全绿 + 用户实机验证 hello 插件热挂载/卸载/设置页 GUI），进入维护态。
