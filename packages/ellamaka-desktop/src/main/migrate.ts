...
// `ellamaka.settings.dat` is special: it maps to the `ellamaka.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"ellamaka.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // ellamaka.settings.dat → the electron settings store ("ellamaka.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "ellamaka.global.dat").
  const storeName = filename === "ellamaka.settings.dat" ? "ellamaka.settings" : filename
