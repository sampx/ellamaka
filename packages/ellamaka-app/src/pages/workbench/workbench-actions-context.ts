...
import {
  ...
  type WorkbenchActionPtyPort,
  type WorkbenchActionSessionPort,
  type WorkbenchActionStorePort,
} from "./workbench-actions"
import { scopeFromTab, scopeName, scopePath } from "./workbench-scope"
...
const WorkbenchActionsContext = createSimpleContext({
  ...
  init: () => {
    ...
    // Task 3 compatibility adapter. Tasks 4-6 migrate callers and replace this
    // adapter with the pure WorkbenchStore and Session Projection ports.
    const store: WorkbenchActionStorePort = {
      ...
      boundPanels: (sessionID) => wb.tabs.flatMap((tab) => {
        const scope = scopeFromTab(tab)
        return (wb.spaceState(tab.path)?.panels ?? [])
          .filter((panel) => panel.slotState === "bound" && panel.boundSessionId === sessionID)
          .map((panel) => ({ scope, panelID: panel.id, panel }))
      }),
      active: () => {
        const active = selectActiveWorkbenchContext({
          spaces: wb.spaces,
          tabs: wb.tabs,
          activeSpaceName: wb.activeSpaceName,
        })
        return active ? { scope: active.scope, panelID: active.panel.id } : undefined
      },
      ...
      commitSessionBinding: (scope, panelID, session) => {
        wb.bindSessionToPanel(scopePath(scope), panelID, session)
      },
      commitSessionUnbinding: (scope, panelID) => wb.unbindSessionFromPanel(scopePath(scope), panelID),
      commitPanelPty: (scope, panelID, kind, ptyID) => {
        wb.setPanelPtyId(scopePath(scope), panelID, kind, ptyID)
      },
    ...
    const pty: WorkbenchActionPtyPort = {
      ...
      disposePty: ({ scope, panelID, kind, knownPtyID }) =>
        ptyManager.disposePty({
          ...
          panelId: panelID,
          kind,
          sdk: sdkForDirectory(wb.spaceState(scopePath(scope))?.panels.find((panel) => panel.id === panelID)?.directory ?? scopePath(scope)),
    ...
    const session: WorkbenchActionSessionPort = {
    ...
    return createWorkbenchActions({ store, pty, session })
  },
})
