...
- Workbench transient notifications must unify under `wb.statusMessage` (i18n keys only), shown at the bottom of the left session tree for 5 seconds and hidden completely when the sidebar collapses. The bottom status bar only presents the current Space, Panel, session, and server context, and must not carry operational help.
