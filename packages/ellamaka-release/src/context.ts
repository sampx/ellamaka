// packages/ellamaka-release/src/context.ts
//
// Single workflow build/publish context. Each release workflow checkouts a
// precise product tag, then assembles one release-context.json that the
// build, manifest, release-notes and upload steps all read. The context is
// not version-controlled; it lives in the workflow run only.
//
// See docs/DISTRIBUTION.md §7.1.

import { buildNamespacedTag, parseNamespacedTag, parseReleaseVersion, buildReleaseIdentity } from "./identity"
 ...
 * already carries).
 */
export function serializeReleaseContext(ctx: unknown): string {
