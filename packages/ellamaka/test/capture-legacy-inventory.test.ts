...
const script = await import("../../../scripts/capture-legacy-release-inventory.mjs")
...
describe("capture-legacy-release-inventory", () => {
  test("dry-run writes a schema-valid inventory with source dry-run", () => {
    ...
    script.main({ dryRun: true, output: outPath })
  ...
  test("classifyTag maps legacy shapes and standard shapes correctly", () => {
    ...
    expect(script.classifyTag("v1.15.13-4")).toEqual({ kind: "legacy", shape: "stable-iteration" })
    expect(script.classifyTag("v1.15.13-1.rc2")).toEqual({ kind: "legacy", shape: "rc-iteration" })
    // New-standard namespaced tags
    expect(script.classifyTag("ellamaka-cli-v1.17.1")).toEqual({ kind: "standard" })
    expect(script.classifyTag("ellamaka-desktop-v1.16.2")).toEqual({ kind: "standard" })
    // Standard SemVer under generic namespace (pre-namespace era)
    expect(script.classifyTag("v1.15.13")).toEqual({ kind: "standard-shape", plain: "1.15.13" })
    expect(script.classifyTag("v1.15.13-beta.4")).toEqual({ kind: "standard-shape", plain: "1.15.13-beta.4" })
    // Truly unparsable
    expect(script.classifyTag("v1.14.39-dev")).toEqual({ kind: "unparsable" })
  })
})
