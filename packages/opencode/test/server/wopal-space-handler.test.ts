...
describe("wopal-space-handler", () => {
  it.instance("default layer executes a capability command", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        Bun.write(
          executable,
          '#!/bin/sh\nprintf \'%s\\n\' \'{"apiVersion":"wopal.capability/v1","capability":"space.list","ok":true,"data":{"items":[{"id":"test-space","name":"test-space","path":"/tmp/test-space","type":"local"}],"total":1}}\'\n',
        ),
      )
      ...
      expect(snapshot.spaces).toEqual([{ id: "test-space", name: "test-space", path: "/tmp/test-space", type: "local" }])
    }),
  )

  test("resolveSpaces returns cached spaces when cache is populated", async () => {
    const cached: SpaceEntry[] = [{ id: "test-space", name: "test-space", path: "/tmp/test", type: "local" }]
  ...
  test("resolveSpaces refreshes through correct CLI path when cache is empty", async () => {
    let capturedPath = ""
    const refreshed: SpaceEntry[] = [{ id: "space-1", name: "space-1", path: "/tmp/space1", type: "local" }]
