...
| Test（重目录集成） | `bun run test:slow` | 修改 server/session/cli/snapshot/project/tool/control-plane 相关行为后 |
...
- 测试子集由 `script/run-tests.ts` 按目录模式展开，`package.json` 提供三个入口：`test:unit`（默认开发子集，扫 `test/` 全部子目录 + 顶层 `*.test.ts`，剔除重目录）、`test:slow`（只跑 7 个重目录集成测试）、`test:all`（全量回归）。日常开发默认用 `test:unit`；改到重目录（server/session/cli/snapshot/project/tool/control-plane）相关代码时至少跑 `test:slow`；提交/合并前跑 `test:all` 回归。
- 新增测试目录默认自动纳入 `test:unit`（扫描式）；若某目录属于慢速集成，把目录名加入 `script/run-tests.ts` 的 `SLOW_DIRS` 常量，并在本文件重目录清单同步。
