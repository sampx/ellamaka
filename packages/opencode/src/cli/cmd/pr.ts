...
export const PrCommand = effectCmd({
  command: "pr <number>",
  describe: `fetch and checkout a GitHub PR branch, then run ${BINARY_NAME}`,
  builder: (yargs) =>
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.body) {
        ...
        if (sessionMatch) {
          ...
          UI.println(`Found ${BINARY_NAME} session: ${sessionUrl}`)
          UI.println(`Importing session...`)

          const importResult = yield* Effect.promise(() =>
            Process.text([BINARY_NAME, "import", sessionUrl], { nothrow: true }),
          )
    ...
    UI.println()
    UI.println(`Starting ${BINARY_NAME}...`)
    UI.println()

    const opencodeArgs = sessionId ? ["-s", sessionId] : []
    const code = yield* Effect.promise(
      () =>
        Process.spawn([BINARY_NAME, ...opencodeArgs], {
          stdin: "inherit",
          stdout: "inherit",
          stderr: "inherit",
          cwd: process.cwd(),
    ...
    if (code !== 0) return yield* Effect.die(new Error(`${BINARY_NAME} exited with code ${code}`))
  }),
})
