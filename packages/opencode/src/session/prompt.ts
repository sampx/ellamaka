...
import { SessionEvent } from "@wopal/core/session-event"
import { ModelV2 } from "@wopal/core/model"
import { ProviderV2 } from "@wopal/core/provider"
import { AgentAttachment, FileAttachment, ReferenceAttachment, Source } from "@wopal/core/session-prompt"
    ...
    const runLoop: (sessionID: SessionID) => Effect.Effect<MessageV2.WithParts> = Effect.fn("SessionPrompt.run")(
      function* (sessionID: SessionID) {
        ...
        while (true) {
          ...
          const isAssistantAfterUser = lastAssistant ? MessageV2.isAfter(lastAssistant, lastUser) : false
          if (
            lastAssistant?.finish &&
            !["tool-calls"].includes(lastAssistant.finish) &&
            !hasToolCalls &&
            (isAssistantAfterUser || lastAssistant.parentID === lastUser.id)
          ) {
