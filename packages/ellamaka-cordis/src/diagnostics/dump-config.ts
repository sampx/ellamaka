...
import { shippedPresetRoot } from "../runtime/anchor.js"
 ...
 *
 * `agent-presets`: assemble the SHIPPED agent-preset root (`standard` etc.)
 * the same way the dsh CLI's `composeProfile` does — `loadProfile` alone does
 * not inject it, so without this the roster is empty and sessions cannot
 * start. The root derives from the same install anchor the profile resolves
 * packages from, so a bundled host reads the materialised closure's presets.
 * The USER root lives under state/.agent-presets (DESIGN-dsh-poc 双根 发现).
 * The stock `includeUserRoot` appends the dsh-home-paths default (~/.dsh or
 * $DSH_HOME), which the integration must never own (constraint #10: never
 * use/set DSH_HOME, never read/write ~/.dsh). We supply the correct user root
 * explicitly and disable the stock one.
 *
  ...
  return [
    {
      ...
      config: {
        ...
        roots: [
          ...
          { path: join(opts.resolvedHome, "state", ".agent-presets"), trust: "user" },
        ],
      },
    },
