...
import { shippedPresetRoot } from "../src/dsh-web"
...
describe("lifted patch builders snapshot equality", () => {
  ...
  test("webExtraPatches matches mountDshWeb original shape exactly", () => {
    const resolvedHome = "/test/resolved/home"
    ...
    expect(patches).toEqual([
      {
        ...
        config: {
          ...
          roots: [
            { path: shippedPresetRoot(undefined), trust: "system" },
