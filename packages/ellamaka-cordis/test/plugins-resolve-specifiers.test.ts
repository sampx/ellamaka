...
describe("resolveRowSpecifier (B1 explicit resolution)", () => {
  test("a closure bare name resolves to its absolute file:// URL", () => {
    ...
    const resolved = resolveRowSpecifier("@deepseek-ai/cordis-plugin-timer", { dshHome: root, installAnchor: anchor })
  ...
  test("a real closure package resolves through the workspace install anchor", () => {
    ...
    const resolved = resolveRowSpecifier("@deepseek-ai/cordis-plugin-timer", { dshHome: home, installAnchor: anchor })
    expect(resolved.startsWith("file://")).toBe(true)
  ...
  test("a profiles-only user plugin resolves via the profiles anchor", () => {
    ...
    const pluginDir = join(root, "profiles", "node_modules", "fx-plugin")
    ...
    const resolved = resolveRowSpecifier("fx-plugin", { dshHome: root, installAnchor: anchor })
    expect(resolved.startsWith("file://")).toBe(true)
  ...
  test("a scoped profiles-only user plugin resolves through the scoped directory", () => {
    ...
    const anchor = fakeClosure(root).anchor
    const pluginDir = join(root, "profiles", "node_modules", "@acme", "widget")
    mkdirSync(pluginDir, { recursive: true })
    ...
    const resolved = resolveRowSpecifier("@acme/widget", { dshHome: root, installAnchor: anchor })
  ...
  test("resolution order is closure first, profiles second", () => {
    ...
    const shadowDir = join(root, "profiles", "node_modules", "@deepseek-ai", "cordis-plugin-timer")
  ...
  test("an absolute file:// URL passes through unchanged", () => {
    ...
    expect(resolveRowSpecifier(url, { dshHome: root, installAnchor: anchor })).toBe(url)
  })

  test("a relative specifier passes through unchanged", () => {
    const root = mkdtempSync(join(tmpdir(), "dsh-resolve-relative-"))
    const anchor = fakeClosure(root).anchor
    expect(resolveRowSpecifier("./index.js", { dshHome: root, installAnchor: anchor })).toBe("./index.js")
  })
  ...
  test("an unresolvable name throws the original error", () => {
    ...
    const anchor = fakeClosure(root).anchor
    try {
      resolveRowSpecifier("ghost-plugin", { dshHome: root, installAnchor: anchor })
      expect.unreachable()
    } catch (error) {
  ...
  test("an unresolvable name still throws without an install anchor", () => {
    ...
    try {
      resolveRowSpecifier("ghost-plugin", { dshHome: root })
      expect.unreachable()
    } catch (error) {
      expect(String(error)).toContain("ghost-plugin")
      expect(String(error)).toMatch(/Cannot find (package|module)/)
